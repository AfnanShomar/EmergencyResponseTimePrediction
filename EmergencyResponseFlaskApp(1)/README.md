...
A **quick-entry** Flask web interface for the trained CatBoost quantile
regression models from `Test1.ipynb`. In a real emergency there is rarely
time to fill in every field a model was trained on, so this app only asks
for the **5 details that genuinely change per incident and matter for a
fast decision**:

1. Distance to incident (km)
2. Incident severity
3. Region type
4. Drone availability
5. Ambulance availability

Every other feature the model needs (traffic, weather, hospital capacity,
battery/fuel level, etc.) is filled in automatically using realistic
defaults — the median (numeric) or most frequent value (categorical)
observed in the real training data — and the timestamp is captured
automatically as the current time instead of being typed in.

It collects this through a web form, runs it through the same
preprocessing logic used in training, and returns Q10 / Q50 / Q90
response-time predictions plus a dispatch recommendation (Hybrid /
Ambulance / Drone / Delayed-Manual Review).
